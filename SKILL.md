...
自动从开头素材第一帧截图 + 叠加项目名称文字，写入剪映草稿封面。

```bash
$ENV_PY "$SCRIPTS/generate_cover.py" "$PROJ" "草稿名-N" --index N
```

**封面参数（固化）：**
| 参数 | 值 |
|------|-----|
| 底图 | 开头素材第一帧，1080×1920竖屏 |
| 文字颜色 | #fdfcb2 (RGB 253/255, 252/255, 178/255) |
| 字体 | 俊雅体（系统回退PingFang） |
| 位置 | 画面居中 |
| 阴影颜色 | #000000 |
| 阴影不透明度 | 60% |
...
**输出：** 直接写入 `com.lveditor.draft/草稿名/draft_cover.jpg`
